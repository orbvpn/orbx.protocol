...
echo -e "${GREEN} Deploying OrbX Protocol VM to ${REGION}${NC}"
echo "=============================================="
...
echo -e "${GREEN}[DONE] Secrets retrieved successfully${NC}"

# Generate UNIQUE WireGuard keys for this region
echo -e "\n${YELLOW} Generating unique WireGuard keys for ${REGION}...${NC}"
WG_PRIVATE_KEY=$(wg genkey)
WG_PUBLIC_KEY=$(echo "$WG_PRIVATE_KEY" | wg pubkey)
echo -e "${GREEN}[DONE] WireGuard keys generated for this region${NC}"
...
if [ "$ORBNET_API_KEY" = "null" ] || [ -z "$ORBNET_API_KEY" ]; then
	...
	if echo "$REGISTER_RESPONSE" | grep -q "already exists"; then
		echo -e "${YELLOW} Server name conflict detected${NC}"
...
if [ "$JWT_SECRET" != "$SHARED_JWT_SECRET" ]; then
	echo -e "${RED} WARNING: Returned JWT secret doesn't match Key Vault!${NC}"
...
echo -e "${GREEN}[DONE] Resource group created${NC}"
...
echo -e "${GREEN}[DONE] Virtual network created${NC}"
...
echo -e "${GREEN}[DONE] Network security group created${NC}"
...
echo -e "${GREEN}[DONE] Public IP created${NC}"
...
echo -e "${GREEN}[DONE] Network interface created${NC}"
...
echo "[DONE] TLS certificate generated"
...
docker run -d \
  ...
  \${ACR_LOGIN_SERVER}/orbx-protocol:prod
...
echo -e "${GREEN}[DONE] Startup script created${NC}"

# ============================================
# Step 6: Create VM
# ============================================
echo -e "\n${YELLOW} Creating virtual machine...${NC}"
...
echo -e "${GREEN}[DONE] Virtual machine created${NC}"
...
echo -e "${GREEN}[DONE] VM FQDN: ${FQDN}${NC}"
echo -e "${GREEN}[DONE] VM IP: ${PUBLIC_IP}${NC}"

# ============================================
# Step 7: Wait for initialization
# ============================================
echo -e "\n${YELLOW}[WAIT] Waiting for VM initialization (this takes 4-5 minutes)...${NC}"
...
echo -e "\n${GREEN}[DONE] Deployment info saved to deployed-servers.txt${NC}"
